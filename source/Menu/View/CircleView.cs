// <copyright file="CircleView.cs" company="LeagueSharp">
...
namespace LeagueSharp.Common.View
{
    ...
    /// </summary>
    [Export(typeof(IView))]
    [ExportMetadata("Service", typeof(MenuItem<Circle>))]
    public class CircleView : View
    {
        ...
        ///     The component.
        /// </param>
        public CircleView(IMenuComponent component)
        {
            this.Component = component as MenuItem<Circle>;
            this.CreateContext();
        }
        ...
        /// <summary>
        ///     Finalizes an instance of the <see cref="CircleView" /> class.
        /// </summary>
        ~CircleView()
        {
        ...
        /// </summary>
        public static CircleView ColorPickerInstance { get; set; }

        /// <summary>
        ///     Gets the component.
        /// </summary>
        public MenuItem<Circle> Component { get; }
        ...
        {
            return new CircleView(component);
        }
        ...
        public override void OnDraw()
        {
            ...
            if (this.Component.Value.Active)
            {
            ...
            switch (message)
            {
                case (uint)WindowsMessages.WM_LBUTTONDOWN:
                    ...
                    if (Utils.IsUnderRectangle(
                        cursor,
                        this.Component.Position.X + width - (height * 2),
                        ...
                        height))
                    {
                        if (!this.IsPickingColor)
                        {
                            DxColorView.ColorPickerInstance?.Close();
                    ...
                    if (Utils.IsUnderRectangle(
                        ...
                        height))
                    {
                        this.Component.Value.Active = !this.Component.Value.Active;
                    }
        ...
        private void CreateContext()
        {
            ...
            CreateGlobalContext((int)height);
            ...
            using (var graphics = Graphics.FromImage(bitmap))
            {
                ...
                SharedView.CreateTitle(graphics, 0, 0, width - (height * 2) - 5, height, title, viewAttributes);
                SharedView.CreateBox(graphics, width - (height * 2), 0, height, height, this.Component.Value.Color);

                if (this.IsPickingColor)
                {
                    SharedView.CreateBackgroundView(graphics, width, 0, Spectrum.Width + 8, Spectrum.Height + 25);
                    using (var pen = new Pen(Color.FromArgb(51, 200, 203, 203)))
                    {
                        var startX = (int)width + 5;
                        var startY = Spectrum.Height + 10;
                        var alphaWidth = Spectrum.Width;
                        var alphaHeight = 10;
                        var alphaValue = this.Component.Value.Color.A;
                        ...
                        {
                            graphics.FillPath(new SolidBrush(Color.FromArgb(150, 200, 203, 203)), path);
                            graphics.DrawPath(pen, path);
                        }

                        using (var path = RoundedRect(new Rectangle(startX, startY, alphaX - startX, alphaHeight), 5))
                        {
                            graphics.FillPath(new SolidBrush(Color.FromArgb(150, 12, 125, 141)), path);
                            graphics.DrawPath(pen, path);
                        }
                        ...
                        {
                            graphics.FillPath(new SolidBrush(Color.FromArgb(255, 0, 0, 0)), path);
                            graphics.DrawPath(pen, path);
                        }
        ...
        private void UpdateColorPicker(Vector2 cursor)
        {
            if (this.ColorPickClick)
            {
                ...
                this.Component.Value.Color = Color.FromArgb(this.Component.Value.Color.A, c.R, c.G, c.B);
                this.updateContext = true;
            }
            else if (this.AlphaPickClick)
            {
                ...
                var c = this.Component.Value.Color;
                this.Component.Value.Color = Color.FromArgb(value, c.R, c.G, c.B);
                this.updateContext = true;
            }
        }
